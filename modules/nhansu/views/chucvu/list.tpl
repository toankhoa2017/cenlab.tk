...
function _load(id) {
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "{site_url()}nhansu/chucvu/ajax_list?id=" + id,
            "type": "POST"
        },
        "columnDefs": [
            {
...
function _save() {
    if ($("#name").val() == "") {
        ...
        $.ajax({
            url: "{site_url()}nhansu/chucvu/ajax_add",
            type: "POST",
            data: $('#form').serialize(),
            dataType: "JSON",
            success: function (data) {
                ...
                if (data.status == 'denied') {
                    swal("{$languages.thatbai}", "{$languages.input1_error}", "error");
                } else {
            ...
            buttons: {
                '{$languages.co}': {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "{site_url()}nhansu/chucvu/ajax_edit",
                            data: {
                                idchucvusua: $("#id_sua").val(),
                                namechucvusua: $("#name").val()
                            },
                            datatype: "text",
                            success: function (data) {
                                if (data == 1) {
        ...
        buttons: {
            'Có': {
                ...
                action: function () {
                    $.ajax({
                        type: "POST",
                        url: "{site_url()}nhansu/chucvu/delete",
                        data: {
                            idchucvu: id
                        },
                        datatype: "text",
                        success: function (data) {
                            if (data == 1) {
                                reload_table();
                                swal("Thành Công!", "Bạn đã xóa chức vụ thành công", "success");
                            } else {
                    ...
                    <div>
                        ...
                        {foreach $listUps as $path}
                            ...
                            {if $stt < count($listUps)}
                                <a href="{site_url()}nhansu/chucvu?id={$path['id']}">{$path['name']}</a>
                                &rightarrow;
                            {else}
                                {$path['name']}
                            {/if}
