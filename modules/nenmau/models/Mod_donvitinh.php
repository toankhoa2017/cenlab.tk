...
class Mod_donvitinh extends MY_Model implements NenmauInterface {

    var $parent = 0;
    var $table = 'mau_donvitinh';
    ...
    function _create($values) {
        $count = 0;
        foreach ($values["loai"] as $key => $val){
            $insert = array(
                'donvitinh_name' => $values['name'],
                'donvitinh_type' => $val['loai'],
                'donvitinh_status' => '1'
            );
    ...
    function suadonvitinh($data, $giongcu) {
        //$kiemtra = $this->db->select('donvitinh_id')->from($this->table)->where("donvitinh_name='" . $data['donvitinh_name'] . "' and donvitinh_type='" . $data['donvitinh_type'] . "' and donvitinh_id!='" . $data['donvitinh_id'] . "' and donvitinh_status='1'")->get();
        //if ($kiemtra->num_rows() == 0) {
