...
{block name=script}
    ...
    function _load() {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "{site_url()}nenmau/kythuat/ajax_list",
                "type": "POST"
            },
            "columnDefs": [
                {
            ...
            buttons: {
                '{$languages.co}': {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "{site_url()}nenmau/kythuat/xoakythuat",
                            data: {
                                idkythuatxoa: id
                            },
                            datatype: "text",
    ...
    var namecu = "";
    function _sua(id, name, mota, name_eng) {
        $('#form')[0].reset(); // reset form on modals
        $('.form-group').removeClass('has-error'); // clear error class
        $('.help-block').empty(); // clear error string
        $('#modal_form').modal('show'); // show bootstrap modal
    ...
    function _sua_oke() {
        if ($("#name").val() == "") {
            swal("{$languages.canhbao}", "{$languages.tenkythuat_validation}", "warning");
        } else {
            $.confirm({
                title: '{$languages.xacnhan}',
                ...
                buttons: {
                    '{$languages.co}': {
                        ...
                        action: function () {
                            $.ajax({
                                type: "POST",
                                url: "{site_url()}nenmau/kythuat/suakythuat",
                                data: {
                                    idkythuatsua: $("#id_sua").val(),
                                    namekythuatsua: $("#name").val(),
                                    motakythuatsua: $("#mota").val(),
                                    tenkythuattruocthaydoi: namecu,
                                ...
                                success: function (data) {
                                    if (data == 1) {
                                        ...
                                        swal("{$languages.thatbai}", "{$languages.tenkythuat_error}", "error");
                                    }
                                }
                            });
                        }
    ...
    function _save() {
        if ($("#name").val() == "") {
            ...
            $.ajax({
                url: "{site_url()}nenmau/kythuat/ajax_add",
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function (data) {
