...
{block name=script}
                ...
                <form id="form" method="post">
                    ...
                    <div class="modal-body form">
                        <div class="form-group">
                            <label>{$languages.tendieukienluu}:</label>
    ...
    function _load() {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "{site_url()}nenmau/dieukienluu/ajax_list",
                "type": "POST"
            },
            "columnDefs": [
                {
            ...
            buttons: {
                '{$languages.co}': {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "{site_url()}nenmau/dieukienluu/xoadieukienluu",
                            data: {
                                iddieukienluuxoa: id
                            },
                            datatype: "text",
    ...
    function _sua_oke() {
        if ($("#name").val() == "") {
            swal("{$languages.canhbao}", "{$languages.tendieukienluu_validation}", "warning");
        } else {
            $.confirm({
                title: '{$languages.xacnhan}',
                ...
                buttons: {
                    '{$languages.co}': {
                        ...
                        action: function () {
                            $.ajax({
                                type: "POST",
                                url: "{site_url()}nenmau/dieukienluu/suadieukienluu",
                                data: {
                                    iddieukienluusua: $("#id_sua").val(),
                                    namedieukienluusua: $("#name").val(),
                                    tendieukienluutruocthaydoi: namecu,
                                },
                                datatype: "text",
                                success: function (data) {
                                    if (data == 1) {
                                        ...
                                        swal("{$languages.thatbai}", "{$languages.tendieukienluu_error}", "error");
                                    }
                                }
                            });
                        }
    ...
    function _save() {
        if ($("#name").val() == "") {
            ...
            $.ajax({
                url: "{site_url()}nenmau/dieukienluu/ajax_add",
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function (data) {
