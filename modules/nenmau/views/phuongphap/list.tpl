...
{block name=script}
                ...
                <form id="form" method="post">
                    ...
                    <div class="modal-body form">
                        <div class="form-group">
                            <label>Mã Phương Pháp:</label>
                            <input autocomplete="off" class="form-control" type="text" id="code" name="code" />
                        </div>
                        <div class="form-group">
                            <label>{$languages.tenphuongphap}:</label>
                            <input autocomplete="off" class="form-control" type="text" id="name" name="name" />
                            <input type="hidden" id="id_sua" name="id_sua">
                        </div>
                        <div class="form-group">
                            <label>{$languages.tenphuongphap_eng}:</label>
                        ...
                        <div class="form-group">
                            <label>Loại Phương pháp</label>
                            <select class="select2 form-control" style="width: 100%" id="loai" name="loai">
                                <option value=""> --Select-- </option>
                                <option value="1"> Tài liệu Nội bộ </option>
                                <option value="2"> Tài liệu Bên ngoài </option>
                            </select>
                        </div>    
                    </div>
    ...
    function _load() {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "{site_url()}nenmau/phuongphap/ajax_list",
                "type": "POST"
            },
            "columnDefs": [
                {
            ...
            buttons: {
                '{$languages.co}': {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "{site_url()}nenmau/phuongphap/xoaphuongphap",
                            data: {
                                idphuongphapxoa: id
                            },
                            datatype: "text",
    ...
    var codecu = "";
    function _sua(id, code, name, mota, name_eng, loai) {
        $('#form')[0].reset(); // reset form on modals
        $('.form-group').removeClass('has-error'); // clear error class
        $('.help-block').empty(); // clear error string
        $('#modal_form').modal('show'); // show bootstrap modal
    ...
    function _sua_oke() {
        if ($("#name").val() == "") {
            swal("{$languages.canhbao}", "{$languages.tenphuongphap_validation}", "warning");
        } else {
            $.confirm({
                title: '{$languages.xacnhan}',
                ...
                buttons: {
                    '{$languages.co}': {
                        ...
                        action: function () {
                            $.ajax({
                                type: "POST",
                                url: "{site_url()}nenmau/phuongphap/suaphuongphap",
                                data: {
                                    idphuongphapsua: $("#id_sua").val(),
                                    codephuongphap: $("#code").val(),
                                    namephuongphapsua: $("#name").val(),
                                    loaiphuongphapsua: $("#loai").val(),
                                    motaphuongphapsua: $("#mota").val(),
                                    //codephuongphaptruocthaydoi: namecu,
                                ...
                                success: function (data) {
                                    if (data == 1) {
                                        ...
                                        swal("{$languages.thatbai}", "{$languages.tenphuongphap_error}", "error");
                                    }
                                }
                            });
                        }
    ...
    function _save() {
        if ($("#name").val() == "") {
            ...
            $.ajax({
                url: "{site_url()}nenmau/phuongphap/ajax_add",
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function (data) {
                        ...
                        <table id="table" class="table table-striped table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 100px;">{$languages.table_2}</th>
                                    <th>{$languages.table_3}</th>
                                    <th>{$languages.table_4}</th>
                                    <th>Loại Phương Pháp</th>
