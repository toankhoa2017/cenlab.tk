...
{block name=script}
                ...
                <form id="form" method="post">
                    ...
                    <div class="modal-body form">
                        <div class="form-group">
                            <label>{$languages.tendonvitinh}</label>
                        ...
                        <div class="form-group loai-phong">
                            ...
                            <select class="select2 form-control" multiple="multiple" style="width: 100%" id="loai" name="loai[]" data-placeholder="Chọn phòng...">
                                <option value="1">{$languages.loaiphong_1}</option>
                                <option value="0">{$languages.loaiphong_2}</option>
                            </select>
                        </div>
                        <div class="form-group loai-phong-read">
                            <label>{$languages.loaidonvi}: <p class="loai-phong-text" style="font-weight: bold"></p></label>
                        </div>    
                    </div>
    ...
    function _load() {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "{site_url()}nenmau/donvitinh/ajax_list",
                "type": "POST"
            },
            "columnDefs": [
                {
            ...
            buttons: {
                '{$languages.co}': {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "{site_url()}nenmau/donvitinh/xoadonvitinh",
                            data: {
                                iddonvitinhxoa: id
                            },
                            datatype: "text",
    ...
    function _sua(id, name, loai) {
        ...
        $(".loai-phong").hide();
        $(".loai-phong-read").show();
        if(loai==0) 
            $(".loai-phong-text").text("{$languages.loaiphong_2}");
        else
            $(".loai-phong-text").text("{$languages.loaiphong_1}");
        $("#name").val(name);
        $("#id_sua").val(id);
        $("#btnSua").show();
        $("#btnSave").hide();
    ...
    function _sua_oke() {
        if ($("#name").val() == "") {
            swal("{$languages.canhbao}", "{$languages.tendonvitinh_validation}", "warning");
        } else {
            $.confirm({
                title: '{$languages.xacnhan}',
                ...
                buttons: {
                    '{$languages.co}': {
                        ...
                        action: function () {
                            $.ajax({
                                type: "POST",
                                url: "{site_url()}nenmau/donvitinh/suadonvitinh",
                                data: {
                                    iddonvitinhsua: $("#id_sua").val(),
                                    namedonvitinhsua: $("#name").val(),
                                    tendonvitinhtruocthaydoi: namecu,
                                    loaidonvitinh: $("#loai").val()
                                },
                                datatype: "text",
                                success: function (data) {
                                    if (data == 1) {
                                        ...
                                        swal("{$languages.thatbai}", "{$languages.tendonvitinh_error}", "error");
                                    }
                                }
                            });
                        }
    ...
    function _save() {
        if ($("#name").val() == "") {
            ...
            $.ajax({
                url: "{site_url()}nenmau/donvitinh/ajax_add",
                type: "POST",
                data: $('#form').serialize(),
                dataType: "JSON",
                success: function (data) {
