...
class Donvitinh extends ADMIN_Controller {
    ...
    function ajax_list() {
        ...
        foreach ($list as $item) {
            ...
            $row[] = htmlentities($item->donvitinh_name);
            if ($item->donvitinh_type == "0") {
                $loai = $ngonngu['loaiphong_2'];
            } else {
                $loai = $ngonngu['loaiphong_1'];
            }
            $row[] = $loai;
            $button = '';
            $button .= ($this->privcheck['update']) ? ' <button class="btn btn-xs btn-info" onclick="_sua(' . $item->donvitinh_id . ',\'' . $item->donvitinh_name . '\',' . $item->donvitinh_type . ')" data-toggle="tooltip" title="'.$ngonngu['tooltip_sua'].'"><i class="ace-icon fa fa-pencil bigger-120"></i></button>' : '';
    ...
    function suadonvitinh() {
        ...
        $loai_donvitinh = $this->input->post("loaidonvitinh");
        $data = array(
            'donvitinh_id' => $id_donvitinh,
            'donvitinh_name' => $name_donvitinh,
            //'donvitinh_type' => $loai_donvitinh,
        );
