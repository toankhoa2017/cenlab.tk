...
{block name=script}
            ...
            <div class="modal-content">
                ...
                <form id="form" method="post">
                    <div class="modal-body form">
                        ...
                        <div class="form-group">
                            <label>Bộ thiết bị</label>
                            <select class="form-control chosen-select" name="deviceset" id="deviceset" >
                                <option value="">Chọn bộ thiết bị</option>
                                {foreach from=$deviceset item=set}
    ...
    function _load() {
        table = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "{site_url()}thietbi/device/ajax_list",
                "type": "POST"
            },
            "columnDefs": [
                {
    ...
    function checkcode() {
		$.ajax({
			url: "{site_url()}thietbi/device/checkcode",
			type: "POST",
			data: $('#code').serialize(),
			dataType:"JSON",
    ...
    function _save() {
        if ($("#code").val() == "") {
            swal("Thông báo", "Mã thiết bị không được rỗng", "warning");
            return false;
        }
        if($("#name").val() == "") {
            swal("Thông báo", "Tên thiết bi không được rỗng", "warning");
            return false;
        }
        if($("#deviceset").val() == "") {
            swal("Thông báo", "Bạn phải chọn bộ thiết bị", "warning");
            return false;
        }
        $.ajax({
            url: "{site_url()}thietbi/device/ajax_add",
            type: "POST",
            data: $('#form').serialize(),
            dataType: "JSON",
            success: function (data) {
    ...
    function edit(id,code,name,idset) {
        ...
        $('.help-block').empty(); // clear error string
        $('#modal_form').modal('show'); // show bootstrap modal
        $('.modal-title').text('Sửa thiết bị mới'); // Set Title to Bootstrap modal title
        $('#deviceid').val(id);
        $('#code').val(code);
        $('#name').val(name);
        $('#deviceset').val(idset);
	...
	function _edit() {
        ...
        $.ajax({
            url: "{site_url()}thietbi/device/update",
            type: "POST",
            data: $('#form').serialize(),
            dataType: "JSON",
            success: function (data) {
    ...
    function del(id) {
		if(confirm("Bạn chắc chắn muốn xóa")) {
            $.ajax({
                url: "{site_url()}thietbi/device/delete",
                type: "POST",
                data: { id : id },
                dataType: "JSON",
                success: function(data) {
        ...
        <div class="main-content-inner">
            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        <!-- PAGE CONTENT BEGINS -->
                        <h3 class="header smaller lighter blue">THIẾT BỊ</h3>
                        <div class="clearfix">
