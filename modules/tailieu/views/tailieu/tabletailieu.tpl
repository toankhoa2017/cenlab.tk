...
<table id="table" class="table table-striped table-bordered table-hover">
    <thead>
        <tr>
            ...
            <th style="width: 110px">Ngày BH</th>
            <th>Xem</th>
        </tr>
    </thead>
    <tbody>
        {foreach from=$items key=k item=val }
            <tr>
                <td style="width: 25%;" class="ellipsis-text"><a href="{site_url()}{$url_action}?id={$val["tai_lieu_id"]}">{$val["tai_lieu_name"]}</a>
                    {if !$val["show_noti"]}
                        <b style="color: red">**</b>
                    {/if}
                </td>
                ...
                <td>{date("d-m-Y", strtotime($val["ngay_ban_hanh"]))}</td>
                <td>
