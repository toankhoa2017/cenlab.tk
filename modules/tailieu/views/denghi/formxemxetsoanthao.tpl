...
<script type="text/javascript">
    ...
    jQuery(function($){
        ...
        $("#de_nghi_ket_qua_id").change(function(){
            if(this.value != 4){ // chấp nhận soạn thảo
                $("#de_nghi_date_start").removeClass("ignore");
                $("#de_nghi_date_end").removeClass("ignore");
                $(".date-start").show();
                $(".date-end").show();
                $(".user-assign").show();
                if(this.value == 3){ // đồng ý soạn thảo
                    $(".date-start label").text("Ngày bắt đầu phê duyệt");
                    $(".date-end label").text("Ngày kết thúc phê duyệt");
                    $(".user-assign label").text("Người phê duyệt");
                    ...
                    $(".date-start label").text("Ngày bắt đầu soạn thảo lại");
                    $(".date-end label").text("Ngày kết thúc soạn thảo lại");
                    $(".user-assign label").text("Người soạn thảo");
                    var userReceive = $('#de_nghi_user_receive');
                    userReceive.val(null).trigger('change');
                    userReceive.find("optgroup").remove().end();
                    userReceive.find("option").remove().end();
                    var option = new Option("--Select--", "", false, false);
                    userReceive.append(option);
                    {foreach from=$soanthao_user key=key item=all_user}
                        var optgroup = $('<optgroup>');
                        optgroup.attr('label', "{$key}");
                        {foreach from=$all_user key=k item=user}
                            var option = new Option('{$user["lastname"]} {$user["firstname"]}', '{$user["id"]}', false, false);
...
<div class="row">
    <div class="col-xs-12">
        ...
        <form class="form-horizontal" role="form" id="validation-form" action="{site_url()}tailieu/denghi/createXemXetSoanThao" method="post" name="createdenghi" id="createdenghi">
            <h3 class="header smaller lighter blue">Xem Xét Bản Thảo</h3>
            {if $validate}
            ...
            <div class="form-group">
                <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> File bản thảo </label>
                <div class="col-sm-6">
                    <div class="input-group">
                        <span class="form-control">
                            {$file_soan_thao->file_name} <a href="{site_url()}{$_UPLOADS_PATH}{$file_soan_thao->file_path}">Download</a>
                        </span>
                    </div>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Kết quả xem xét </label>
                <div class="col-sm-6">
            ...
            <div class="form-group date-start ignore">
                <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Ngày bắt đầu phê duyệt</label>
                <div class="col-sm-6">
                    <div class="input-group">
            ...
            <div class="form-group date-end ignore">
                <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Ngày kết thúc phê duyệt</label>
                <div class="col-sm-6">
                    <div class="input-group">
            ...
            <div class="form-group user-assign">
                <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Người phê duyệt </label>
                <div class="col-sm-6">
